...
*				Based on original KBASE by Cameron
*
...
Version 4.1 and 4.2
Added simple mode so that you only need to create one category and sub category (to maintain compatibility) and the front page goes straight into the KBASEs. Changed te version number (because 34 was a bit big)

Version 3.34 and 3.33
Multiple image upload (Maybe enhance this in next release or maybe not)
Fixed bug in admin new KBASE
Category icons
...
Top xx kbases can be shown in menu
Turn off Random KBASE
For BKONE - some compatibility with the old KBASE plugin by Cameron
This is an upgrade from 3.31.
Database validity check file
Integrity check file
KBASE views now shown. 
...
******** approve all existing KBASEs before they are visible.
...
+++++++++++
	...
	+ Changed all KBASEs references to KBASE
...
+++++++++++++++++
	...
	+ Separated Search en KBASE from the language file E107 (gave them their own)
...
+++++++++++++++++++
	...
	+ Search Feature added. (thanks for ReneJM for some of the code)
        + KBASE "Answer" is displayed on a separate page to avoid long pages. 
        + Other minor visual enhancements. 
	
	To-Do-List
	==========
	+ Multi-Language (thanks scaramousch - your files included)
	+ edit button on the kbase "answer" page for admins only.
