...
{
    $kbase_show = true;
    if ($kbasedivsl = $e107cache->retrieve("nq_kbasenew_menu"))
    {
        $kbase_text .= $kbasedivsl;
    }
    else
    {
        $kbase_arg = "select kbase_id,kbase_question,kbase_views,kbase_parent from #kbase left join #kbase_info on kbase_parent = kbase_info_id where kbase_approved>0 and find_in_set(kbase_info_class,'" . USERCLASS_LIST . "') order by kbase_datestamp desc limit 0," . $kbase_obj->kbase_topnum;
        if ($sql->db_Select_Gen($kbase_arg))
        {
            $kbase_cache .= "<div class='forumheader3' style='text-align:center;'><strong>" . KBASELAN_137 . " " . $kbase_obj->kbase_topnum . " " . KBASELAN_136 . "</strong><br /><br />";
            while ($kbase_row = $sql->db_Fetch())
            {
                ...
                $kbase_cache .= "<a href='" . e_PLUGIN . "kbase/kbase.php?0.cat." . $kbase_parent . "." . $kbase_id . "' title='" . htmlentities($tp->toHTML($kbase_question),ENT_QUOTES) . "' >" . htmlentities($tp->toHTML($kbase_quest)) . "</a> <br />";
            }
            $kbase_cache .= "</div>";
        }
        else
        ...
        $e107cache->set("nq_kbasenew_menu", $kbase_cache);
...
$kbase_text .= "</div>";
$ns->tablerender($tp->toFORM($KBASE_PREF['kbase_title']), $kbase_text,'kbase_newmenu');
